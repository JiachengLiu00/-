    ...
    for batch_idx, (inputs, targets) in enumerate(tqdm(train_loader)):
        ...
        if atk is not None:  #
            if atk_mode.lower() == 'fgsm':
                atk.set_training_mode(model_training=False, batchnorm_training=False, dropout_training=False)
                rpst = representation(inputs).clone().detach().to(inputs) #inputs本身
                rpst.requires_grad = True
                loss = criterion(model(rpst).mean(0), targets)
                grad = torch.autograd.grad(loss, rpst, #计算梯度
                                    retain_graph=False, create_graph=False)[0]
                adv_rpst = rpst + atk.eps * grad.sign()  #依据梯度对图像进行改变
                adv_rpst = torch.clamp(adv_rpst, min=0, max=1).detach()#图像大小限制在0-1之间
                rpst = adv_rpst
            else:
                pass # mix
        else:
