...
parser.add_argument('-atk_m', '--attack_mode', default='FGSM', type=str, help='attack mode (_, FGSM, HIRE)')
...
# class VoxelGridMean(VoxelGrid): #体素网格
...
def main():
    ...
    log_dir = '%s-checkpoints' % (args.dataset) #在字符串里输入内容
    if not os.path.exists(log_dir):
        os.makedirs(log_dir)

    seed_all(args.seed)
    if 'cifar' in args.dataset.lower(): #转成小写
    ...
    def replace(model):  #替换网络里的子模块_modules的LIFSpike，加上门控机制。
        for name, module in model._modules.items(): #递归地调用 replace 函数。这确保了所有子模块及其子模块都被处理。
    ...
    def get_mean_p(modules):  #获得均值泊松编码 nn.Hardtanh
        x = 0
        for m in modules:
    ...
    if len(args.attack_mode) > 0:  #实例化atk
    ...
    for epoch in range(args.epochs):
        ...
        if args.allow_sparse:
            scheduler2.step()
        tmp, _ = val(model, test_loader, device, rpst, args.time,atk=atk)
        if args.allow_sparse:
