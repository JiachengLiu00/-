...
from models.inhibition import *
from models.utils import *

class TensorNormalization(nn.Module):  #归一化模块
...
def normalizex(tensor, mean, std):  #归一化
...
class ZIF(torch.autograd.Function):  #激活函数
    @staticmethod
    def forward(ctx, input, gama):  #正向传播正常0，1
        out = (input >= 0).float()
        L = torch.tensor([gama])
    ...
    @staticmethod
    def backward(ctx, grad_output): #反向传播用代理函数计算梯度
...
class RateBp(torch.autograd.Function):  #待定
    @staticmethod
    def forward(ctx, x):
        mem = 0.
        spike_pot = []
...
class LIFSpike(nn.Module):  #LIF模型
...
class ConvexCombination(nn.Module):  #计算输入张量的加权和，权重由 self.comb 参数决定，表示每个输入张量在最终结果中的贡献。权重是可训练的，允许模型在训练过程中学习最佳的权重分配。
    def __init__(self, n):
        super().__init__()
        self.n = n
