...
class PoissonCoding(torch.autograd.Function): #泊松编码
    @staticmethod
...
class inhibition_module(torch.autograd.Function):  #抑制模型 进行门口控制的模块 论文中主要贡献
    @staticmethod
...
class SparsePoissonCoding(nn.Module): # version 2 #门控模块
    def __init__(self, T, size=None):
